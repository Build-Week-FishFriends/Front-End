   ...
   useEffect(() => {
    ...
    return (
     ...
     <label>Fish Name
            ...
            {touched.fishId && errors.fishId && (
         <p className="error">{errors.fishId}</p>
         )}
         </label>
     <label> Fish Count
         ...
         <p className="error">{errors.fishCount}</p>
     )}
     </label>
     <label>Time Spent
...
const FormikLogForm = withFormik({
    ...
    mapPropsToValues({baitType, waterBodyId, fishId, fishCount, timeSpent, timeOfDay }) {
        return {
            baitType: baitType || "",
